...
class Window:
    def __init__(self, sim, config={}):
        ...
        self.vehicle_rate_input_text = ""  # Campo de entrada de texto para la tasa de vehículos
        self.vehicle_rate_input_rect = pygame.Rect(1220, 0, 100, 20)  # Rectángulo que define el área del campo de entrada de texto
            ...
            for event in pygame.event.get():
                ...
                # Handle mouse events
                    ...
                    if event.key == pygame.K_RETURN:
                        # El usuario presionó Enter, actualizar vehicleRate con el valor ingresado
                        try:
                            new_vehicle_rate = int(self.vehicle_rate_input_text)
                            self.sim.vehicleRate = new_vehicle_rate
                        except ValueError:
                            print("Valor de tasa de vehículos no válido")
    ...
    def draw_status(self):
        ...
        if current_time - self.last_print_time >= 60:
            ...
            text_average_vehicles_per_minute = self.text_font.render(f'Average Vehicles Per Minute={int(vehicles_passed/self.sim.t*60)}', False, (0, 0, 0))
            # Print the data
            print(f"t: {self.sim.t:.5f}, n: {self.sim.frame_count}, Vehicles Passed: {vehicles_passed}, Vehicles Present: {self.sim.vehiclesPresent}, Average Vehicles Per Minute: {int(vehicles_passed/self.sim.t*60)}, Total Vehicles: {vehicles_passed + self.sim.vehiclesPresent}, Vehicle Rate: {self.sim.vehicleRate}")
    ...
    def draw(self):
        ...
        # Dibujar el campo de entrada de texto para vehicleRate
        vehicle_rate_text = self.text_font.render("Vehicle Rate:", True, (0, 0, 0))
        self.screen.blit(vehicle_rate_text, (1090, 0))
        pygame.draw.rect(self.screen, (0, 0, 0), self.vehicle_rate_input_rect, 2)
        vehicle_rate_input = self.text_font.render(self.vehicle_rate_input_text, True, (0, 0, 0))
        self.screen.blit(vehicle_rate_input, (1220, 0, 100, 20))
